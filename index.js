    ...
    <style>
        * {
        ...
        .admin-password-input {
            ...
            color: var(--text-primary);
            font-size: 16px;
            margin-bottom: 16: border-color 0.3s;
        }

        .admin-password-inputpx;
            transition:focus {
            outline: none;
...
<body>
    ...
    <script>
        ...
        // Get displayed songs (with pagination)
        ...
        function loadMoreSongs() {
            ...
            setTimeout(() => {
                displayedSongsCount += loadMoreCount;
                isLoadingMore = false;
                renderContent();
            }, 300);
        }
        ...
        function renderSearchResults() {
            ...
            return \`
                <div class="search-results-header">
                    <h1>Search Results</h1>
                    <p class="search-results-count">Showing \${displayedSongs.length} of \${filteredSongs.length} songs for "\${searchQuery}"</p>
                </div>
                <div class="song-grid">
                ...
                \${renderLoadMoreButton(filteredSongs.length)}
            \`;
        }

        // Render load more button
        function renderLoadMoreButton(totalSongs) {
            if (!hasMoreSongs()) return '';

            return \`
                <div class="load-more-container">
                    <button class="load-more-btn" id="loadMoreBtn" onclick="loadMoreSongs()">
                        \${isLoadingMore ? '<div class="loading-spinner"></div> Loading...' : 'Load More Songs'}
                    </button>
                </div>
            \`;
        }

        // Modified renderHome to include search results and pagination
        function renderHome() {
        ...
        function handleSeeAllClick(event) {
            ...
            // Reset pagination when going to library
            displayedSongsCount = 50;
                    ...
                    <div class="song-list-header">
                        \${searchQuery ? \`
                            <div class="search-results-header" style="margin: 0;">
                                <h1>Search Results</h1>
                                <p class="search-results-count">Showing \${displayedSongs.length} of \${filteredSongs.length} songs</p>
                            </div>
                        \` : \`
                            <h1>Your Library</h1>
                        \`}

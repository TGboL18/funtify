  ...
  async fetch(request, env) {
    try {
      ...
      if (pathname === '/setwebhook' && request.method === 'GET') {
        ...
        if (!webhookUrl) {
          return new Response('Please provide ?url=YOUR_WEBHOOK_URL', { status: 400 });
        }
        const setUrl = `https://api.telegram.org/bot${TELEGRAM_TOKEN}/setWebhook?url=${encodeURIComponent(webhookUrl)}`;
...
async function handleTelegramWebhook(request, env) {
  ...
  if (body.callback_query) {
    ...
    if (data === 'start') {
      await answerCallbackQuery(queryId, 'Welcome to Funtify! 🎵');
    }
...
I'll reply with processing status.`;
      ...
      await sendMessage(chatId, welcomeAdmin, null, keyboard);
    } else {
...
Click below to visit:`;
      ...
      await sendMessage(chatId, welcomeUser, null, keyboard);
    }
...
Visit Funtify to listen to music!`;
  ...
  if (text === '/myid') {
    await sendMessage(chatId, `Your Telegram ID: <code>${userId}</code>`, messageId);
    return new Response('ok', { status: 200 });
  }
  ...
  if (!audio && !forwardedFrom) {
    ...
    if (isAdmin) {
      await sendMessage(chatId, `❓ I don't understand. Send me an audio file, voice message, or document to upload.`, messageId);
    }
    return new Response('ok', { status: 200 });
  }
  ...
  // Validate file
  if (!fileId) {
    await sendMessage(chatId, `❌ Could not get file ID.`, messageId);
    return new Response('ok', { status: 200 });
  }
  ...
  try {
    ...
    const song = {
      id: songId,
      title: fileName,
      artist: message.from ? `${message.from.first_name || ''} ${message.from.last_name || ''}`.trim() || 'Admin' : 'Admin',
      tg_file_id: fileId,
      duration: duration,
      mime: mimeType,

...
<body>
    ...
    <script>
        ...
        // Pagination state
        let displayedSongsCount = 0;
        let isLoadingMore = false;
        const INITIAL_SONGS = 50;
        const INITIAL_SEARCH_SONGS = 30;
        ...
        function initLiveViewerCount() {
            // Initialize with random count
            liveViewerCount = Math.floor(Math.random() * 500) + 100;
            updateViewerCount();
            
            // Update every 5 seconds
            setInterval(() => {
                // Add random variance between -10 and +10
        ...
        function setupSearch() {
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                // Reset pagination for search and re-render content with search filter
                resetPagination();
                renderContent();
            });
        ...
        // Get songs to display based on pagination
        function getDisplaySongs() {
            const filtered = getFilteredSongs();
            return filtered.slice(0, displayedSongsCount);
        }
        
        // Check if there are more songs to load
        function hasMoreSongs() {
            const filtered = getFilteredSongs();
            return displayedSongsCount < filtered.length;
        }
        ...
        function loadMoreSongs() {
            ...
            isLoadingMore = true;
            const filtered = getFilteredSongs();
            const moreSongs = filtered.slice(displayedSongsCount, displayedSongsCount + SONGS_PER_PAGE);
            
            // Update the displayed count
            displayedSongsCount += moreSongs.length;
            
            // Re-render to show more songs
            renderContent();
        ...
        function resetPagination() {
            // Use different initial count for search vs normal view
            displayedSongsCount = searchQuery ? INITIAL_SEARCH_SONGS : INITIAL_SONGS;
            isLoadingMore = false;
        }
        ...
        function renderSearchResults() {
            ...
            return `
                <div class="search-results-header">
                    <h1>Search Results</h1>
                    <p class="search-results-count">Showing ${displaySongs.length} of ${filteredSongs.length} songs for "${searchQuery}"</p>
                </div>
                <div class="song-grid">
                ...
                ${hasMoreSongs() ? '<div class="load-more-container" style="text-align: center; padding: 20px;"><button class="load-more-btn" onclick="loadMoreSongs()" style="background: var(--accent); color: #000; border: none; padding: 12px 24px; border-radius: 500px; font-weight: bold; cursor: pointer;">Load More Songs</button></div>' : ''}
            `;
        }
            ...
            return `
                ...
                <section class="section">
                    <div class="section-header">
                        ...
                        <a href="/song/list" class="see-all" id="seeAllLink" onclick="handleSeeAllClick(event)">See all</a>
                    </div>
                    <div class="song-grid">
                        ${displaySongs.map((song, index) => `
        ...
        // Modified renderSongList to include search and pagination
                    ...
                    <div class="song-list-header">
                        ${searchQuery ? `
                            <div class="search-results-header" style="margin: 0;">
                                <h1>Search Results</h1>
                                <p class="search-results-count">Showing ${displaySongs.length} of ${filteredSongs.length} songs found</p>
                            </div>
                        ` : `
                            <h1>Your Library</h1>
                        `}
        ...
        async function router() {
            ...
            // Reset pagination when routing to a new page
            resetPagination();
        ...
        function init() {
            ...
            // Set up infinite scroll for pagination
            setupInfiniteScroll();
    ...
    <script>
        ...
        async function loadAdminStats() {
            try {
                ...
                document.getElementById('currentViewers').textContent = viewers;
            } catch (error) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // API Routes
      if (path === '/api/songs') {
        // Get all songs metadata from both worker patterns
        const songs = [];
